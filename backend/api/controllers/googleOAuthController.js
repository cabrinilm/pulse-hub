...
async function handleGoogleCallback(req, res) {
    try {
        ...
        const createdEvent = await (0, googleCalendarService_1.addEventToGoogleCalendar)(accessToken, event);
        // Redireciona para frontend com mensagem e link
        const frontendUrl = "http://localhost:5173/google-callback";
        return res.redirect(`${frontendUrl}?message=${encodeURIComponent("Event successfully added to Google Calendar!")}&eventLink=${encodeURIComponent(createdEvent.htmlLink || "")}`);
    }
    catch (err) {
        console.error("Google OAuth Controller Error:", err.response?.data || err.message);
        const frontendUrl = "http://localhost:5173/google-callback";
        return res.redirect(`${frontendUrl}?error=${encodeURIComponent("Failed to add event to Google Calendar: " + err.message)}`);
    }
}
