...
class signupsController {
    ...
    async createSignup(req, res) {
        try {
            ...
            const signup = await signupsModel_1.default.createSignup(supabase, user_id, event_id);
            res.status(201).json(signup);
        }
        catch (error) {
            if (error instanceof Error) {
                if (error.message === "Event not found or not authorized" ||
    ...
    async addUserToEvent(req, res) {
        try {
            ...
            const addedSignup = await signupsModel_1.default.addUserToEvent(supabase, creator_id, event_id, username);
    ...
    async listSignups(req, res) {
        try {
            ...
            const signups = await signupsModel_1.default.listSignups(supabase, user_id);
    ...
    async listEventSignups(req, res) {
        try {
            ...
            const stats = await signupsModel_1.default.getEventSignupStats(supabase, event_id);
    ...
    async updateSignup(req, res) {
        try {
            ...
            const updatedSignup = await signupsModel_1.default.updateSignup(supabase, user_id, event_id, updates);
